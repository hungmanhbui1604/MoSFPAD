...
from losses import SVCHingeLoss
...
def evaluate(
    ...
    for images, labels in pbar:
        ...
        preds = torch.where(logits.squeeze(1) >= 0, 1, -1)
...
def main(cfg: dict, no_wandb: bool = False, checkpoint: str = None) -> None:
    ...
    # ── Transforms ────────────────────────────────────────────────────────
    train_transform, eval_transform = get_transforms(data_cfg["transform_name"])
    ...
    # Loss
    criterion = SVCHingeLoss(C=loss_cfg["C"])
    ...
    for epoch in epoch_pbar:
        ...
        if is_main():
            metrics = evaluate(_unwrap(model), val_loader, criterion, device, epoch)
